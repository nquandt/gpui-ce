...
unsafe fn enumerate_contacts(
    ...
    unsafe {
        ...
        // SAFETY: this block runs synchronously on the calling thread inside
        // `enumerateContactsWithFetchRequest:error:usingBlock:` below, so
        // `contacts_ptr` (derived from the `&mut Vec<Contact>` borrowed by
        // the enclosing call) stays valid and uniquely accessed for the
        // block's lifetime.
